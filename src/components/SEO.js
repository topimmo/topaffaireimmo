...
export function useSEO(props) {
    useEffect(() => {
        const { title = 'TopAffaireImmo - Trouvez votre propriété parfaite au Maroc', description = 'TopAffaireImmo est la plateforme immobilière de référence au Maroc. Trouvez des appartements, maisons, villas et propriétés commerciales à vendre ou à louer.', keywords = 'immobilier Maroc, propriété Maroc, appartements Maroc, vente Maroc, location Maroc, Casablanca, Rabat, Marrakech', canonical, ogTitle, ogDescription, ogImage = 'https://topaffaireimmo.vercel.app/og-image.jpg', ogType = 'website', noindex = false, structuredData, } = props;
        ...
        // Hreflang tags
        const hreflangFr = document.querySelector('link[hreflang="fr-MA"]');
        const hreflangAr = document.querySelector('link[hreflang="ar-MA"]');
        if (!hreflangFr) {
